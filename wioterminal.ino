...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect(clientID.c_str(), mqtt_username, mqtt_password)) {
      ...
      Serial.println(TOPIC_PUB_MOTION);

      client.subscribe(TOPIC_PUB_MOTION);
...
void setup() {
  ...
  Serial.println(ssid);
  show("Connected to WiFi successfully");
  delay(500);
...
void loop() {
  ...
  if (digitalRead(PIR_MOTION_SENSOR)) {
    ...
    Serial.println("PIR Motion Sensor: Watching");
    client.publish(TOPIC_PUB_MOTION,"Watching");
  }
